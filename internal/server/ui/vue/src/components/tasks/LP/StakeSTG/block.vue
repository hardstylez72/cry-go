...
export default defineComponent({
  ...
  data() {
    return {
      selectedToken: Token.STG,
      networks: [Network.POLIGON] as Network[],
      tokens: [Token.STG] as Token[],
      item: {
        add: true,
        network: Network.POLIGON,
        amount: {
          sendAll: true,
        },
        tokens: [{token: Token.STG}]
      } as DefaultLP,
    }
  },
  methods: {
    ...
    getTask(): Task {
      const task = {
        ...
        stakeSTG: this.item,
        taskType: TaskType.StakeSTG,
        description: ''
      }
      task.description = taskProps[TaskType.StakeSTG].descFn(task)
    ...
    syncTask() {
      if (this.task) {
        if (this.task.stakeSTG) {
          this.item = this.task.stakeSTG
