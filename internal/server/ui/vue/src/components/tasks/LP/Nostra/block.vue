...
export default defineComponent({
  name: "TaskZkLendLP",
  components: {TokenSelector, NetworkSelector, AmountInput, WEIInputField},
  emits: ['taskChanged'],
  props: {
    weight: {
      type: Number,
  ...
  data() {
    return {
      ...
      networks: [Network.StarkNet] as Network[],
      tokens: [Token.ETH, Token.USDC, Token.USDT] as Token[],
      item: {
        network: Network.StarkNet,
        amount: {
          sendAll: true,
        },
        tokens: [{token: Token.USDC}]
      } as DefaultLP,
    }
  },
  methods: {
    ...
    getTask(): Task {
      const task = {
        ...
        nostraLPTask: this.item,
        taskType: TaskType.NostraLP,
        description: ''
      }
      task.description = taskProps[TaskType.NostraLP].descFn(task)
    ...
    syncTask() {
      if (this.task) {
        if (this.task.nostraLPTask) {
          this.item = this.task.nostraLPTask
