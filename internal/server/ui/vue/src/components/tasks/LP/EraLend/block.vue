...
export default defineComponent({
  ...
  data() {
    return {
      ...
      tokens: [Token.ETH] as Token[],
      item: {
        network: Network.ZKSYNCERA,
        amount: {
          sendAll: true,
        },
        tokens: [{token: Token.ETH}]
      } as DefaultLP,
    }
  },
  methods: {
    ...
    getTask(): Task {
      const task = {
        ...
        eralendLPTask: this.item,
        taskType: TaskType.EraLend,
        description: ''
      }
      task.description = taskProps[TaskType.EraLend].descFn(task)
    ...
    syncTask() {
      if (this.task) {
        if (this.task.eralendLPTask) {
          this.item = this.task.eralendLPTask
