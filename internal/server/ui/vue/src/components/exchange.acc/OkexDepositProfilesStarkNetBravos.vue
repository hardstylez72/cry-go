<template>
  <div>
    ...
    <v-autocomplete
      ...
      item-value="id"
      label="Выберите Braavos профиль"
      density="compact"
      no-data-text="нет свободных Braavos профилей"
    />
  </span>
  </div>
</template>
...
export default defineComponent({
  name: "OkexDepositProfilesStarkNetBravos",
      ...
      try {
        await withdrawerService.withdrawerServiceOkexDepositAddrDetach({
          body: {
            ...
            subType: ProfileSubType.Braavos
          }
        })
    ...
    async searchProfiles() {
      ...
      this.timer.cb(async () => {
        try {
          ...
          const res = await profileService.profileServiceSearchProfilesNotConnectedToOkexDeposit({body: {subType: ProfileSubType.Braavos}})
