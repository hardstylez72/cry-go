<template>
  <div>
    ...
    <v-autocomplete
      ...
      item-value="id"
      label="Выберите UrgentX профиль"
      density="compact"
      no-data-text="нет свободных UrgentX профилей"
    />
  </span>
  </div>
</template>
...
export default defineComponent({
  name: "OkexDepositProfilesStarkNetUrgentX",
      ...
      try {
        await withdrawerService.withdrawerServiceOkexDepositAddrDetach({
          body: {
            ...
            subType: ProfileSubType.UrgentX
          }
        })
    ...
    async attach(p: Profile) {
      if (p && p.id) {
        ...
        try {
          ...
          this.deposit.subType = p.subType
    ...
    async searchProfiles() {
      ...
      this.timer.cb(async () => {
        try {
          ...
          const res = await profileService.profileServiceSearchProfilesNotConnectedToOkexDeposit({body: {subType: ProfileSubType.UrgentX}})
