...
func (t *MaverickSwapTask) Run(ctx context.Context, a *Input) (*v1.ProcessTask, error) {
	...
	if p.GetTx().GetTxId() == "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, "EstimateMaverickSwapCost")
		}
		res, gas, err := MaverickSwap(taskContext, profile, p, client, estimation)
		if err != nil {
			return nil, errors.Wrap(err, "MaverickSwap")
		}
...
func MaverickSwap(ctx context.Context, profile *halp.Profile, p *v1.MaverickSwapTask, client zksyncera.MaverickSwapper, estimation *v1.EstimationTx) (*bozdo.DefaultRes, *bozdo.Gas, error) {
